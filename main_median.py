...
def main(sessionName, trialName, trial_id, cameras_to_use=['all'],
         ...
         # forceSyncActivity: override automatic activity detection for sync.
    ...
    runSynchronization = bool(runSynchronization)
    # Triangulation.
    ...
    # With runSynchronization=False, loadPresynchronizedVideosAndFilter() runs
    # after OpenPose (when runPoseDetection=True) and skips time-shift sync.
    if runTriangulation and not runSynchronization and not runPoseDetection:
        raise RuntimeError(
            "Triangulation needs 2D keypoints. Enable runPoseDetection, or set "
            "runSynchronization=True, or disable runTriangulation."
        )
        
    # %% Paths and metadata. This gets defined through web app.
    baseDir = os.path.dirname(os.path.abspath(__file__))
    user_session_root = dataDir
    if user_session_root is None:
        dataDir = getDataDirectory(isDocker)
        sessionDir = os.path.normpath(os.path.join(dataDir, sessionName))
    else:
        sessionDir = os.path.normpath(os.path.join(user_session_root, sessionName))
        if not os.path.exists(os.path.join(sessionDir, 'sessionMetadata.yaml')):
            parent_dir = os.path.normpath(os.path.join(user_session_root, os.pardir))
            alt_session_dir = os.path.normpath(os.path.join(parent_dir, sessionName))
            if os.path.exists(os.path.join(alt_session_dir, 'sessionMetadata.yaml')):
                sessionDir = alt_session_dir
    ...
    if runCameraCalibration:    
        ...
        for pathCam in glob.glob(os.path.join(sessionDir, 'Videos', 'Cam*')):
            camName = os.path.basename(pathCam.rstrip('/\\'))
    ...
    elif runPoseDetection:
        # OpenPose just ran; load pkls without running synchronizeVideos().
        logging.info(
            "Skipping synchronization - assuming videos are already synchronized")
    ...
    if runOpenSimPipeline:
        ...
        if scaleModel:
            ...
            popNeutralPoseImages(cameraDirectories, cameras2Use, 
                                 ...
                                 trial_id, input_media_trial_name=trialName,
                                 writeVideo=True)   
